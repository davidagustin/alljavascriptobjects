...
// bad@: false
// good@test.co.uk: true`
        },
        {
          title: "Extract Data with Groups",
          description: "Use capture groups to extract specific parts of matched text",
          code: `const phoneRegex = /\\((\\d{3})\\)\\s?(\\d{3})-(\\d{4})/;
const text = 'Call me at (555) 123-4567 or (888) 999-0000';

// Find all phone numbers
const phoneNumbers = [...text.matchAll(new RegExp(phoneRegex.source, 'g'))];
...
phoneNumbers.forEach(match => {
  ...
  console.log('---');
        ...
        {
          title: "Text Replacement with Functions",
          description: "Use functions in replace() for dynamic text transformation",
          code: `const text = 'The quick brown fox jumps over the lazy dog';

// Convert to title case
const titleCase = text.replace(/\\b\\w+\\b/g, word => 
  word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()
);
...
const numberText = 'I have 3 cats and 2 dogs';
const wordNumbers = {
  '1': 'one', '2': 'two', '3': 'three', 
  '4': 'four', '5': 'five'
};

const result = numberText.replace(/\\d/g, match => 
  wordNumbers[match] || match
);

console.log(result);
// Output: 'I have three cats and two dogs'`
        }
      ]}
    />
  )
